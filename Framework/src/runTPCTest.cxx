...
void customize(std::vector<ConfigParamSpec>& workflowOptions)
{
  workflowOptions.push_back(
    ConfigParamSpec{ "test-option", VariantType::Bool, false, { "This option does nothing!" } });
}
...
{
  WorkflowSpec specs;
  bool noDS = config.options().get<bool>("test-option");
  ...
  DataProcessorSpec producer{
    ...
    Inputs{},
    Outputs{
      { "TPC", "RAWDATA", 0, Lifetime::Timeframe }
    },
    AlgorithmSpec{
      (AlgorithmSpec::InitCallback) [](InitContext&) {
        ...
        return (AlgorithmSpec::ProcessCallback) [generator](ProcessingContext& processingContext) mutable {
          ...
          auto data = processingContext.outputs().make<char>(Output{ "TPC", "RAWDATA", 0, Lifetime::Timeframe },
                                                             length);
          for (auto&& item : data) {
  ...
  std::string filename = "tpcTest.json";
